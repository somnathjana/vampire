...
inline std::string expand_str(std::string str){
   ...
   if(str == "s") return "system_magnetization_squared";
   if(str == "g") return "grain_magnetization_squared";
   if(str == "m") return "material_magnetization_squared";
...
// Constructor
//------------------------------------------------------------------------------------------------------
//magnetization_squared_statistic_t::magnetization_squared_statistic_t (){}

//------------------------------------------------------------------------------------------------------
// Function to initialize data structures
//------------------------------------------------------------------------------------------------------
void magnetization_squared_statistic_t::initialize(stats::magnetization_statistic_t& mag_stat) {

   // Check that magnetization statistic is properly initialized
   if(!mag_stat.is_initialized()){
      terminaltextcolor(RED);
      std::cerr << "Programmer Error - Uninitialized magnetization statistic passed to magnetization_squared statistic - please initialize first." << std::endl;
      terminaltextcolor(WHITE);
      zlog << zTs() << "Programmer Error - Uninitialized magnetization statistic passed to magnetization_squared statistic - please initialize first." << std::endl;
      err::vexit();
   }
   ...
   mean_magnetization_squared.resize(4*num_elements,0.0);
...
//------------------------------------------------------------------------------------------------------
// Function to calculate the mean of the square of the magnetisation
//
//       <m_l^2> 
//
//       m_l = sum_i mu_i S_i
...
//
//-------------------------------------------------------------------------------------------------------
void magnetization_squared_statistic_t::calculate(const std::vector<double>& magnetization){
   ...
   for(int id=0; id< num_elements; ++id){
      ...
      mean_magnetization_squared[4*id + 0]+=mx*mx*mm*mm;
      mean_magnetization_squared[4*id + 1]+=my*my*mm*mm;
      mean_magnetization_squared[4*id + 2]+=mz*mz*mm*mm;
      mean_magnetization_squared[4*id + 3]+=mm*mm;
...
//------------------------------------------------------------------------------------------------------
// Function to write mean magnetization_squared data to a checkpoint file
//------------------------------------------------------------------------------------------------------
void magnetization_squared_statistic_t::save_checkpoint(std::ofstream& chkfile){

   const uint64_t num_elements = mean_magnetization_squared.size();
   ...
   chkfile.write(reinterpret_cast<const char*>(&mean_magnetization_squared[0]),sizeof(double)*mean_magnetization_squared.size());
...
// Function to write mean magnetization_squared data to a checkpoint file
//------------------------------------------------------------------------------------------------------
void magnetization_squared_statistic_t::load_checkpoint(std::ifstream& chkfile, bool chk_continue){
   ...
   std::vector<double> read_mean_magnetization_squared(num_elements, 0.0);

   // read data elements
   chkfile.read((char*)&read_mean_counter,sizeof(double));
   //chkfile.read((char*)&read_mean_susceptibility[0],sizeof(double)*num_elements);
   chkfile.read((char*)&read_mean_magnetization_squared[0],sizeof(double)*num_elements);
   ...
   if(chk_continue){
      ...
      if(num_elements == mean_magnetization_squared.size()){
         ...
         mean_magnetization_squared = read_mean_magnetization_squared;
...
//------------------------------------------------------------------------------------------------------
// Function to reset magnetization_squared averages
//------------------------------------------------------------------------------------------------------
void susceptibility_statistic_t::reset_averages(){
   ...
   std::fill(mean_magnetization_squared.begin(),mean_magnetization_squared.end(),0.0);
...
//------------------------------------------------------------------------------------------------------
// Function to output mean magnetization_squared values as string
//------------------------------------------------------------------------------------------------------
std::string magnetization_squared_statistic_t::output_mean_magnetization_squared(const double temperature, bool header){
   ...
   if(!header){
      ...
      for(int id=0; id< num_elements - 1; ++id){ // ignore last element as always contains non-magnetic atoms
         ...
         const double msq_x = mean_magnetization_squared[4*id + 0]*imean_counter;
         const double msq_y = mean_magnetization_squared[4*id + 1]*imean_counter;
         const double msq_z = mean_magnetization_squared[4*id + 2]*imean_counter;
         const double msq_m = mean_magnetization_squared[4*id + 3]*imean_counter;

         result << msq_x << msq_y << msq_z << msq_m;

      }
   }
   else {
      for(int id=0; id< num_elements - 1; ++id){ // ignore last element as always contains non-magnetic atoms
         result << name + std::to_string(id) + "_msq_x"
                << name + std::to_string(id) + "_msq_y"
                << name + std::to_string(id) + "_msq_z"
                << name + std::to_string(id) + "_msq_m";
      }
   }
   return result.str();
